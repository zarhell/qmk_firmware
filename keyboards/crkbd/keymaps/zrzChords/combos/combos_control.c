...
const uint16_t PROGMEM z_x_lsft_caps[] = { KC_Z,  KC_X, LALT_T(KC_SCLN), COMBO_END }; // Z+X+Alt(;) → Caps
const uint16_t PROGMEM app_menu_combo[] = { KC_M,  KC_N,         COMBO_END }; // M+N → Context menu
const uint16_t PROGMEM alt_tab_combo[]  = { SFT_T(KC_TAB), KC_A, COMBO_END }; // Tab+A → Alt+Tab
