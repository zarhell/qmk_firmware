...
const uint16_t PROGMEM left_acute[]  = { KC_D, KC_R,          COMBO_END };  // D+R → ´
const uint16_t PROGMEM left_comment[]= { KC_X, KC_D, KC_F,    COMBO_END };  // X+D+F → DEV_COMMENT (Ctrl+/)
const uint16_t PROGMEM left_tild[]  = { KC_Q, KC_F, COMBO_END };
const uint16_t PROGMEM left_quote[] = { KC_W, KC_R, COMBO_END };  // W+R → '
